...
# V: Return an array of all links that contain an eventID.
sub linksForEventID {
    my ($self, $eid) = @_;
	my @links = ();
	foreach $link ($self->tlinks()) {
		push @links, $link if $self->linkContainsInstance($link, $eid);
	}
	return @links;
}
...
#V: link contains event with certain event id
sub linkContainsEvent {
    my ($self, $link, $eid) = @_;
	my $relatedToEvent = $link->{'relatedToEvent'};
	my $subordinatedEvent = $link->{'subordinatedEvent'};
	if ($relatedToEvent) {
		return 1 if $relatedToEvent eq $eid;
	} elsif ($subordinatedEvent) {
		return 1 if $link->{'subordinatedEvent'} eq $eid;
	}
	return 0;
}
