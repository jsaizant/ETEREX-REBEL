...
sub fromTimemlTags {
    ...
    $objref->{_eid1} = $tlink->{'fromEID'};
    $objref->{_eid2} = $tlink->{'toEID'};
    $objref->{_tid1} = $tlink->{'fromTID'};
    $objref->{_tid2} = $tlink->{'toTID'};
...
sub from_opening_tag
{
    ...
    #$objref->{_rel} = $1 if $string =~ /relType="(\S+)"/;
    $objref->{_rel} = $1 if $string =~ /type="(\S+)"/;
    #$objref->{_eiid1} = $1 if $string =~ /eventInstanceID="(\S+)"/;
    $objref->{_eid1} = $1 if $string =~ /fromEID="(\S+)"/;
    #$objref->{_eiid2} = $1 if $string =~ /relatedToEventInstance="(\S+)"/;
    $objref->{_eid2} = $1 if $string =~ /toEID="(\S+)"/;
    #$objref->{_conf} = $1 if $string =~ /confidence="(\S+)"/;
    #$objref->{_tid1} = $1 if $string =~ /timeID="(\S+)"/;
    $objref->{_tid1} = $1 if $string =~ /fromTID="(\S+)"/;
    #$objref->{_tid2} = $1 if $string =~ /relatedToTime="(\S+)"/;
    $objref->{_tid2} = $1 if $string =~ /toTID="(\S+)"/;
...
sub id1 {
	my $self = shift;
	$self->eid1 ? $self->eid1 : $self->tid1 }

sub id2 {
	my $self = shift;
	$self->eid2 ? $self->eid2 : $self->tid2 }
